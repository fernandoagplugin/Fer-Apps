...
st.set_page_config(page_title="EquityDash Ultra v6.5", page_icon=LOGO_SIDEBAR, layout="wide")
...
acoes_config = {
    'AXIA6.SA': {'cor': '#3bb54a', 'payout': 1.0, 'lpa': 6.80, 'vpa': 52.10, 'price': 68.65, 'logo': "https://raw.githubusercontent.com/fernandoagplugin/LOGOS/0261825cda3f92616b4c36e82cf5201588429c74/AXIA.png"},
    'CPLE3.SA': {'cor': '#2d3e50', 'payout': 0.5, 'lpa': 0.85, 'vpa': 10.40, 'price': 15.90, 'logo': "https://raw.githubusercontent.com/fernandoagplugin/LOGOS/0261825cda3f92616b4c36e82cf5201588429c74/COPEL.png"},
    'CXSE3.SA': {'cor': '#005ca9', 'payout': 0.9, 'lpa': 1.40, 'vpa': 4.10, 'price': 18.09, 'logo': "https://raw.githubusercontent.com/fernandoagplugin/LOGOS/0261825cda3f92616b4c36e82cf5201588429c74/Caixa.png"},
    'ITSA4.SA': {'cor': '#ec7000', 'payout': 0.4, 'lpa': 1.55, 'vpa': 8.90, 'price': 13.92, 'logo': "https://raw.githubusercontent.com/fernandoagplugin/LOGOS/0261825cda3f92616b4c36e82cf5201588429c74/Itausa.png"},
    'SAPR4.SA': {'cor': '#009fe3', 'payout': 0.5, 'lpa': 1.10, 'vpa': 6.80, 'price': 7.88, 'logo': "https://raw.githubusercontent.com/fernandoagplugin/LOGOS/0dd7c40bf47a5487a468aeaca985451e8d24cc6a/Sanepar.PNG"}
}
...
if oports and valor_aporte > 0:
    ...
    for idx, c in enumerate(oports):
        qtd = (valor_aporte / len(oports)) // c['p']
        a_cols[idx].metric(f"Comprar {c['t'][:5]}", f"{int(qtd)} cotas", f"R$ {qtd*c['p']:.2f}")

# 7. Gráfico
st.markdown("<br>", unsafe_allow_html=True)
target = st.selectbox("Comparativo:", list(acoes_config.keys()), index=4)
fig = go.Figure()
...
for t in [target, 'BOVA11.SA', 'DIVO11.SA']:
    ...
    if not df.empty:
        ...
        if not df.empty:
            ...
            fig.add_trace(go.Scatter(x=df.index, y=norm, name=t[:6]))

fig.update_layout(template="plotly_white", height=450, margin=dict(l=0, r=0, t=10, b=10))
