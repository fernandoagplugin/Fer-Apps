...
# --- CSS AVANÇADO (Visual Moderno Mantido) ---
st.markdown("""
    <style>
...
st.markdown('<div class="main-header"><h1>EquityDash Ultra</h1><p>Valuation de Graham & Bazin + Alocação Inteligente</p></div>', unsafe_allow_html=True)
...
# 5. Processamento dos Cards e Calculos
calculos_ativos = []
cols = st.columns(4)
...
# 6. Sugestão de Alocação (Corrigida)
st.markdown("---")
...
if oportunidades and valor_aporte > 0:
    ...
    st.info("Nenhuma oportunidade de compra identificada com os parâmetros atuais.")

# 7. Gráfico Comparativo (Corrigido)
...
with st.container():
    ...
    if target in market_data:
        ...
        def plot_series(t, is_main=False):
            ...
            df_f = df[df.index >= pd.to_datetime(data_corte)]
            if not df_f.empty:
                ...
                fig.add_trace(go.Scatter(x=df_f.index, y=norm, name=t[:6], 
                                         line=dict(color=color, width=3 if is_main else 1, dash=None if is_main else 'dot')))
