...
for ticker, conf in acoes_config.items():
    with st.sidebar.expander(f"Ajustar {ticker[:5]}"):
        ...
        lpa_p = st.number_input(f"LPA ({ticker[:5]})", value=float(s_lpa), step=0.05, key=f"in_lpa_{ticker}")
...
# 5. Gráfico com Formatação de Hover (Mouse)
st.markdown("---")
...
if ticker_sel in dados_mercado:
    ...
    # Formato do hover: %{y:.2f}% (mostra 2 casas e o símbolo %)
    # Se quiser 5 casas, mude para %{y:.5f}%
    ...
    fig.add_trace(go.Scatter(x=d['datas'], y=d['divo_norm'], name='DIVO11', 
                             hovertemplate=hover_fmt, line=dict(color='#95a5a6', dash='dash')))
    
    fig.add_trace(go.Scatter(x=d['datas'], y=cdi_norm, name='CDI', 
                             hovertemplate=hover_fmt, line=dict(color='#e74c3c', dash='dot')))
    
    fig.update_layout(
        template="plotly_white", 
        hovermode="x unified", 
        height=500,
        yaxis=dict(ticksuffix="%"), # Adiciona % no eixo Y
